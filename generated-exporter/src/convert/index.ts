...
import { formatFlatDimension, formatStructuredDimension, formatDuration, isPxOrRemUnit } from "../util/units"
...
export function convertToken(
  ...
  const base = (dtcgType: DtcgType, $value: unknown, extensions?: Record<string, unknown>): DtcgToken => {
  ...
  if (tokenType === "color") {
    ...
    const $value = config.valueFormat === "structured" ? toStructuredColor(v) : toHexString(v)
  ...
  if (DIMENSION_FAMILY_TYPES.has(tokenType)) {
    ...
    if (!isPxOrRemUnit(v.unit)) {
      warnings.push(`"${name}" (${tokenType}) has unit "${v.unit}", which DTCG's dimension type doesn't support (px/rem only). Exported as $type: "number" instead.`)
      return { token: base("number", v.measure), warnings }
    }
    const $value = config.valueFormat === "structured" ? formatStructuredDimension(v.measure, v.unit) : formatFlatDimension(v.measure, v.unit)
  ...
  if (tokenType === "duration") {
    const v = value as { measure: number; unit: string }
    return { token: base("duration", formatDuration(v.measure, v.unit)), warnings }
  }
  ...
  if (tokenType === "gradient") {
    ...
    warnings.push(...result.warnings.map((w) => `"${name}": ${w}`))
    return { token: base("gradient", result.value, result.extensions), warnings }
  }

  // --- No DTCG equivalent at all: textCase, textDecoration, visibility, blur --
  if (!config.includeUnmappableTokenTypes) {
    warnings.push(`"${name}" (${tokenType}) has no DTCG equivalent and includeUnmappableTokenTypes is off -- skipped.`)
    return { token: null, warnings }
  }
  warnings.push(`"${name}" (${tokenType}) has no DTCG equivalent -- exported as $type: "string" with the raw value tagged in $extensions.`)
  const rawText = typeof value === "object" && value && "value" in value ? String((value as any).value) : JSON.stringify(value)
  return { token: base("string", rawText, { originalTokenType: tokenType }), warnings }
}
